...
1. **Propose ADR-0041** for v0.16.0: C5 binary format becomes the default. v0.15.0 JSON stays available as a fallback for cross-language consumers or any caller for whom human-readability matters more than load time. The empirical performance curve (1.85× at 10k, 2.93× at 100k) is documented honestly in the ADR and in the v0.16.0 release notes — no "build once, deploy 100× faster" overstatement. **Done — ADR-0041 Accepted, v0.16.0 tagged.** Public API: `MarshalCompiledSnapshot` / `UnmarshalCompiledSnapshot` on `engine/indexed`.
2. **Re-run E3/E4/E5** for C5 (determinism, 100k round-trip, adversarial). The harness already does correctness checks at 10k and 100k; the v0.15.0-style determinism + adversarial suite should be re-run against C5 before promotion. **Partially done** — correctness verified at both scales via the orchestrator; a v0.15.0-style determinism + adversarial battery against the promoted API would be a worthwhile pre-tag check for future v0.16.x releases.
...
4. **Document the realistic perf curve in the v0.16.0 README and cookbook.** **Done** — the v0.16.0 CHANGELOG, README, and cookbook carry the measured curve and explicitly say "not 'build once, deploy 100× faster.'"
