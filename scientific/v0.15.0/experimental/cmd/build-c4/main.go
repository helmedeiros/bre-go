...
func main() {
	var (
		...
		out  = flag.String("out", "snapshot.c4.bin", "snapshot output filename within -dir")
		exec = flag.Bool("exec", false, "execute inputs.jsonl and emit results-c4-source.jsonl")
	)
	flag.Parse()
	...
	pre, err := e.ExportPreClassifiedRules()
	if err != nil {
		die(err)
	}
	...
	if err := format.EncodeC4(f, pre); err != nil {
		_ = f.Close()
		die(err)
	}
	...
	if *exec {
		...
		if err := format.WriteResultsJSONL(filepath.Join(*dir, "results-c4-source.jsonl"), results); err != nil {
			die(err)
		}
	}
	fmt.Printf("build-c4: wrote %s (%d rules)\n", *out, len(pre))
}

func die(err error) {
	fmt.Fprintln(os.Stderr, "build-c4:", err)
	os.Exit(1)
}
