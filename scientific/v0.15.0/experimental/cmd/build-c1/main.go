...
func main() {
	var (
		...
		out    = flag.String("out", "snapshot.c1.json", "compact-JSON snapshot output filename within -dir")
		exec   = flag.Bool("exec", false, "execute inputs.jsonl and emit results-c1-source.jsonl")
	)
	flag.Parse()
	...
	if err := format.EncodeC1(f, snap); err != nil {
		_ = f.Close()
		die(err)
	}
	...
	if *exec {
		...
		if err := format.WriteResultsJSONL(filepath.Join(*dir, "results-c1-source.jsonl"), results); err != nil {
			die(err)
		}
	}

	fmt.Printf("build-c1: wrote %s (%d rules)\n", *out, len(snap.Rules))
	_ = json.NewEncoder
}

func die(err error) {
	fmt.Fprintln(os.Stderr, "build-c1:", err)
	os.Exit(1)
}
