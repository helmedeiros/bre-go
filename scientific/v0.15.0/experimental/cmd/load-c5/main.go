...
func main() {
	var (
		dir      = flag.String("dir", ".", "directory containing snapshot.c5.bin (and inputs.jsonl if -exec)")
		snap     = flag.String("snapshot", "snapshot.c5.bin", "snapshot filename within -dir")
		trials   = flag.Int("trials", 1, "number of timing trials")
		exec     = flag.Bool("exec", false, "execute inputs.jsonl and emit results-c5.jsonl")
		timings  = flag.String("timings", "", "write per-trial timings (ns, one per line) to this path")
		label    = flag.String("label", "load-c5", "label printed on timing summary line")
	)
	flag.Parse()
	...
	if *exec {
		...
		if err := format.WriteResultsJSONL(filepath.Join(*dir, "results-c5.jsonl"), results); err != nil {
			die(err)
		}
	}
}
...
func loadOnce(path string) (*indexed.Engine, error) {
	...
	return indexed.LoadCompiledSnapshot(cs, nil)
}

func writeTimings(path string, ts []time.Duration) {
...
func die(err error) {
	fmt.Fprintln(os.Stderr, "load-c5:", err)
	os.Exit(1)
}
