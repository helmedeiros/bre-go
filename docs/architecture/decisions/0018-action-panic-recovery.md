...
`OnExecutionFinished` always fires, even on the error path, so listeners using duration metrics can still record the elapsed time of a failed execution. The order on the error path is: `OnRuleMatched` (for any rules that completed *before* the panicking one) → `OnExecutionErrored` → `OnExecutionFinished`. The contract case pins this order.

`OnRuleMatched` is deliberately **not** fired for the panicking rule itself. The rule's name still appears in `Result.Matched` (it did match its `Condition`), but the match *event* is reserved for successful Action completion. Listeners that count matches via `OnRuleMatched` (`CountingListener`, `TimingListener.MatchesInLastExecution()`) therefore reflect only the rules whose Actions ran to completion. Callers needing "this rule's condition was true" semantics regardless of Action outcome read `Result.Matched` instead.
