...
`v0.19.0` -- nineteenth minor release. Adds **port-level sentinel errors**: `engine.ErrEmptyRuleName` and `engine.ErrDuplicateRuleName` are now canonical at the port level; each adapter's existing per-package sentinel wraps the engine-level one via `fmt.Errorf("<adapter>: %w", engine.ErrXxx)`. A single `errors.Is(err, engine.ErrEmptyRuleName)` check handles all four adapters; pre-v0.19 consumers using the per-adapter sentinel keep working unchanged; error message strings are byte-identical. Closes one of the three "soft spots" ADR-0044's stability audit named. ADR-0045 Accepted; forty-five ADRs on `main`. SemVer: pre-1.0 means breaking changes are still allowed but land as a `0.x → 0.(x+1)` minor bump; see [ADR-0021](docs/architecture/decisions/0021-release-versioning-policy.md).

```sh
go get github.com/helmedeiros/bre-go@v0.19.0
```
...
- **`engine.ErrEmptyRuleName`, `engine.ErrDuplicateRuleName`.** Stable since v0.19.0. Port-level umbrella sentinels for the two `AddRule` validation errors every adapter shares. `errors.Is(err, engine.ErrEmptyRuleName)` works across all four adapters; the per-adapter checks (`errors.Is(err, inmemory.ErrEmptyRuleName)` etc.) keep working unchanged because each adapter's sentinel wraps the engine-level one. Error message strings are byte-identical to pre-v0.19. See [ADR-0045](docs/architecture/decisions/0045-engine-level-sentinel-errors.md).
- **Per-adapter registration validation (`<adapter>.ErrEmptyRuleName`, `<adapter>.ErrNilCondition` / `<adapter>.ErrNilMatch`, `<adapter>.ErrDuplicateRuleName`).** Stable. Same shape-first-then-state-second check order on every adapter. The two cross-adapter sentinels now wrap the engine-level umbrella; `ErrNilCondition` / `ErrNilMatch` stay adapter-specific because the underlying field names differ.
...
- **`engine/indexed.Engine`.** Stable since v0.8.0; widened in v0.9.0, v0.10.0, v0.11.0; lifecycle added in v0.12.0. Sub-linear first-match adapter. Rules carry `Match parser.Condition`. Indexable shapes (bucket-key contributors): `StringCondition{OpEq}` (v0.8.0), `SetCondition{OpIn}` (v0.9.0, fan-out). Post-filter shapes: `StringCondition{OpNeq}`, `SetCondition{OpNotIn}` (v0.10.0), `RangeCondition` (v0.11.0). Custom typed Conditions via `Engine.WithPostFilterHook(h)` (v0.11.0). Each rule needs at least one indexable term; pure-post-filter rules return `ErrNoIndexableTerms`. Unknown shapes return `ErrNonIndexableCondition`; fan-outs above `maxFanout` (1024) return `*FanoutTooLargeError`. Execute is O(K) hash lookups + per-candidate post-filter evaluation. **Concurrent-safe Execute** (v0.12.0): after `Build()` (or the first implicit Build via Execute), any number of goroutines can call Execute simultaneously, lockless on the read path. Subsequent `AddRule` returns `ErrEngineBuilt`. Live perf-bar tests gate any future change that would weaken the headline claim.
- **`engine/indexed.Engine.Build()` / `Built()`.** Stable since v0.12.0. Explicit lifecycle: `Build()` finalizes the rule set into an immutable snapshot held in `sync/atomic.Value`; subsequent `Execute` runs lockless. `Built()` reports the state. Hot reload is a caller-side pattern: build a new engine, swap an `atomic.Value` in caller code. The library doesn't ship a wrapper type; the cookbook documents the four-line pattern.
- **`observability.StructuredTelemetryListener`** + `TelemetryRecord` + `TelemetrySink`. Stable since v0.13.0. Wire on any adapter via `e.AddListener(observability.NewStructuredTelemetryListener(sink))`. The listener implements every lifecycle interface and emits one record per terminal event (Finished / Errored) via the caller-supplied sink. Two records per Execute on the error path; sinks correlate if needed.
- **`engine/indexed.Engine.Diagnose()`** + `DiagnoseReport` + `DeadRule`. Stable since v0.14.0. Static rule-set analyzer: reports rules that can never fire because an earlier rule shadows them. Conservative tier-1 check skips pairs where the earlier rule has post-filter terms (`OpNeq`, `OpNotIn`, `RangeCondition`, hook-classified conditions) -- false positives are zero by design. Works in both engine phases (pre-Build / post-Build) without triggering implicit Build. O(N² × F); call from startup validation or an admin endpoint, not per-request.
- **`engine/indexed.Engine.ExportSnapshot()`** + `LoadSnapshot()` + `Snapshot` + `SnapshotRule` + `SnapshotCondition` + `RuleCallbacks` + `SnapshotFormatVersion`. Stable since v0.15.0. JSON-serializable snapshot of a built engine's rule set; LoadSnapshot reconstructs an already-Built engine. Hook-bearing engines refuse to export (`ErrSnapshotIncompatibleHook`); custom typed Conditions need a separate protocol. Action callbacks attach out-of-band at Load time via a name-keyed registry. Strict FormatVersion gate (`ErrSnapshotFormatVersionMismatch`) -- no migration shims in v0.15.0. **Not a load-time optimization**: at 10k rules the measured median speedup vs. CSV + `parser.ParseToCondition` is 0.49× (slower). The feature's proven dimensions are content-addressable determinism, cross-architecture portability, and adversarial round-trip integrity — see [`scientific/v0.15.0/REPORT.md`](scientific/v0.15.0/REPORT.md). For performance-sensitive consumers, prefer the v0.16.0 binary format below.
- **`engine/indexed.Engine.ExportCompiledSnapshot()`** + `LoadCompiledSnapshot()` + `MarshalCompiledSnapshot()` + `UnmarshalCompiledSnapshot()` + `CompiledSnapshot` + `CompiledBucket` + `CompiledRuleRef` + `FieldValueSet` + `PreClassifiedRule` + `CompiledSnapshotFormatVersion`. Stable since v0.16.0. **Binary, pre-bucketed snapshot format**: serializes the engine's compiled internal state so the loader skips `AddRule` and `Build` entirely. Measured median 1.85× faster than CSV + `parser.ParseToCondition` at 10 000 rules, **2.93× at 100 000 rules**, with per-rule scaling that degrades only 30% from 10k to 100k vs source-build's 210%. 50% smaller on disk than v0.15.0 JSON. Cross-arch portable (arm64 ↔ amd64 byte-identical match results). Strict version gate (`ErrCompiledSnapshotFormatVersionMismatch`) -- no migration shims. Hook-bearing engines refuse to export with `ErrSnapshotIncompatibleHook`, same contract as v0.15.0 JSON. Action callbacks attach out-of-band by rule name. The intermediate `PreClassifiedRule` form (and `AddPreClassifiedRule` constructor) is the fallback for callers loading rules from a pre-processed source who don't want the full bucket-state encoding. Full measurements in [`scientific/v0.15.0/experimental/REPORT.md`](scientific/v0.15.0/experimental/REPORT.md); design rationale in [ADR-0041](docs/architecture/decisions/0041-binary-snapshot-format.md).
- **`observability/otel.Wrap(inner, tracer, opts...)`** + `WithSpanName(name)` + `AttrAdapter` / `AttrMatchedCount` / `AttrMatchedNames` / `AttrCorrelationID` / `AttrCanceled` / `AttrCancelReason` constants. Stable since v0.17.0. Decorator that emits one OpenTelemetry span per `Execute` on any `engine.Engine`. Matched rule names go on the span as a string-slice attribute; correlation IDs from `engine.WithCorrelationID(ctx, id)` are tagged automatically; **cancellation is distinguished from failure** (`context.Canceled` / `context.DeadlineExceeded` set `canceled = true` + a `cancel.reason` enum, with span status left `Unset`); other errors record the typed exception and set `codes.Error`. The wrapper exposes `RuleNames()`, `RuleInfos()`, `AddListener()`, and `Unwrap()` and forwards them to the inner engine when supported. Lives in `observability/otel/` inside the main module, pinned to `go.opentelemetry.io/otel` v1.11.x for Go-1.18 compatibility. Design rationale in [ADR-0042](docs/architecture/decisions/0042-opentelemetry-adapter.md); pre-tag scientific review in [`scientific/v0.17.0/REPORT.md`](scientific/v0.17.0/REPORT.md).
- **`observability.ExecutionMetric`** + **`observability.ExecutionMetricSink`** + **`observability/metrics.Wrap`** + **`observability/metrics.RecordingSink`**. Stable since v0.18.0. The **hexagonal metrics port and decorator**: bre-go owns the typed metric event and the sink interface; backends adapt to it. The decorator times Execute, classifies the outcome (success / true error / cancellation), and emits one `ExecutionMetric` per call to whatever sink the caller provides. `Err` and `(Canceled, CancelReason)` are mutually exclusive — the v0.17 cancellation-is-not-error lesson encoded into the data model. `RecordingSink` is a thread-safe reference implementation; consumers wanting OTel-native metrics implement their own sink (~50 LOC against the port) against the upstream OTel metric API. Capability forwarding (`RuleNames`, `RuleInfos`, `AddListener`, `Unwrap`) mirrors the v0.17 OTel span decorator. Design rationale in [ADR-0043](docs/architecture/decisions/0043-metrics-port-and-decorator.md); pre-tag scientific review (three independent sinks, ~150 LOC total) in [`scientific/v0.18.0/REPORT.md`](scientific/v0.18.0/REPORT.md).
- **`engine/parser.ParseValueExpression(field, value)`.** Stable since v0.10.0. CSV-shaped value cell → typed Condition. `"BR"` → `OpEq`, `"!BR"` → `OpNeq`, `"BR|AR"` → `OpIn`, `"*"` or empty → `nil`. Errors via `*ValueExpressionError` for mixed forms, empty operands, empty alternatives.
- **`engine/parser.RangeCondition{Field, Min, Max}`.** Stable since v0.11.0. Inclusive numeric range over float64; `Eval` parses the input field via `strconv.ParseFloat`. Infinity bounds (`math.Inf(±1)`) for half-open intervals.

What may still change:

- **Adapter-internal `Rule` struct field order.** Today's adapters keep `Name` first by convention. The struct shape is stable as-is.
...
| [`engine/indexed`](engine/indexed) | Sub-linear first-match. Rules are bucketed by their key-set + value-tuple; `Execute` runs O(K) hash lookups where K is the number of distinct key-sets. Indexable shapes (bucket-key contributors): `StringCondition{OpEq}`, `SetCondition{OpIn}`. Post-filter shapes: `StringCondition{OpNeq}`, `SetCondition{OpNotIn}`, `RangeCondition` (v0.11.0). Caller-defined custom shapes via `Engine.WithPostFilterHook`. Wildcard semantics via field omission. Input must be `map[string]string` or `map[string]interface{}`. | You have a large rule set (hundreds to tens of thousands of rules) shaped as routing-table / decision-table equality / set / negation / range matches, and per-call latency matters. See [`BENCHMARKS.md`](BENCHMARKS.md) for the cleared v0.8.0 / v0.9.0 / v0.10.0 / v0.11.0 success bars. |
