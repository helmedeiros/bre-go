...
`v0.16.0` -- sixteenth minor release. Adds the **binary "compiled snapshot" path**: `ExportCompiledSnapshot` + `LoadCompiledSnapshot` (Go types) + `MarshalCompiledSnapshot` + `UnmarshalCompiledSnapshot` (wire format) on `engine/indexed`. Encodes the engine's already-bucketed state so the loader skips `AddRule` and `Build`. Measured **1.85× faster than CSV + `parser.ParseToCondition` at 10 000 rules, 2.93× faster at 100 000 rules** (full distribution in [`scientific/v0.15.0/experimental/REPORT.md`](scientific/v0.15.0/experimental/REPORT.md)); 50% smaller files than v0.15.0 JSON; cross-arch portable (arm64 ↔ amd64 verified). The v0.15.0 JSON snapshot path stays available for cross-language consumers and human-readable use cases. Forty-one ADRs on `main`. SemVer: pre-1.0 means breaking changes are still allowed but land as a `0.x → 0.(x+1)` minor bump; see [ADR-0021](docs/architecture/decisions/0021-release-versioning-policy.md).

```sh
go get github.com/helmedeiros/bre-go@v0.16.0
```
