...
`v0.12.0` -- twelfth minor release. Adds the build-then-execute lifecycle to `engine/indexed` (`Build()` / `Built()`), making `Execute` safe for concurrent calls from arbitrary goroutines via an atomic.Value-held snapshot. Listener fan-out across all four adapters becomes concurrent-safe via copy-on-write. Hot reload becomes a documented one-liner using `atomic.Value` in caller code -- no new library type. Backward-compatible: existing v0.8.0-v0.11.0 callers continue to work via implicit Build on first Execute. Fourth of five Phase-4 follow-ups (see [ADR-0037](docs/architecture/decisions/0037-build-then-execute-and-concurrent-safe-execute.md)). Four concrete adapters, rule loading from CSV and JSON, twelve public packages plus one test-only sibling, thirty-seven ADRs on `main`. SemVer: pre-1.0 means breaking changes are still allowed but land as a `0.x → 0.(x+1)` minor bump; see [ADR-0021](docs/architecture/decisions/0021-release-versioning-policy.md).

```sh
go get github.com/helmedeiros/bre-go@v0.12.0
```
