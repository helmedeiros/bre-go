...
`v0.11.0` -- eleventh minor release. Adds numeric-range matching (`parser.RangeCondition`) to `engine/indexed` as a post-filter, plus a small extension hook (`Engine.WithPostFilterHook`) so callers can register their own typed-Condition shapes without forking. Two new v0.11.0 success-bar cells clear at 100× / 2 936× firstmatch; v0.8.0 + v0.9.0 + v0.10.0 bars continue to gate. Third of five Phase-4 follow-ups closing the gap toward production-grade matcher shapes (see [ADR-0036](docs/architecture/decisions/0036-indexed-range-condition-and-custom-post-filter.md)). Four concrete adapters, rule loading from CSV and JSON, twelve public packages plus one test-only sibling, thirty-six ADRs on `main`. SemVer: pre-1.0 means breaking changes are still allowed but land as a `0.x → 0.(x+1)` minor bump; see [ADR-0021](docs/architecture/decisions/0021-release-versioning-policy.md).

```sh
go get github.com/helmedeiros/bre-go@v0.11.0
```
