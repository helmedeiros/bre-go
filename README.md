...
`v0.15.0` -- fifteenth minor release. Adds `engine/indexed.Engine.ExportSnapshot()` and `LoadSnapshot()` -- a JSON-serializable snapshot of an indexed engine's rule set, plus a matching loader that produces an already-Built engine ready to Execute. Build-once / deploy-many lifecycle: a build job loads the authoritative source, exports an artifact; consumer processes load it and skip every parse / validate / canonicalize cost. Action callbacks attach out-of-band by rule name at Load time. Four concrete adapters, rule loading from CSV and JSON, twelve public packages plus one test-only sibling, forty ADRs on `main`. SemVer: pre-1.0 means breaking changes are still allowed but land as a `0.x → 0.(x+1)` minor bump; see [ADR-0021](docs/architecture/decisions/0021-release-versioning-policy.md).
