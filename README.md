...
`v0.13.0` -- thirteenth minor release. Adds a built-in `observability.StructuredTelemetryListener` that emits a typed `TelemetryRecord` per Execute via a caller-supplied sink. Wire it on any of the four adapters via `e.AddListener(...)`; it implements every lifecycle interface and bundles input / output / matched / duration / err into one struct. Four concrete adapters, rule loading from CSV and JSON, twelve public packages plus one test-only sibling, thirty-eight ADRs on `main`. SemVer: pre-1.0 means breaking changes are still allowed but land as a `0.x → 0.(x+1)` minor bump; see [ADR-0021](docs/architecture/decisions/0021-release-versioning-policy.md).
