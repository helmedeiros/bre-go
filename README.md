...
`v0.14.0` -- fourteenth minor release. Adds `engine/indexed.Engine.Diagnose()` -- a static rule-set analyzer that reports rules which can never fire because an earlier rule shadows them. Conservative tier-1 check (zero false positives): skips earlier rules carrying post-filter terms, so a clean Diagnose doesn't *prove* zero dead rules but does prove zero dead rules under shadowers without post-filters. Useful at startup or admin-endpoint; not on the per-request hot path. Four concrete adapters, rule loading from CSV and JSON, twelve public packages plus one test-only sibling, thirty-nine ADRs on `main`. SemVer: pre-1.0 means breaking changes are still allowed but land as a `0.x → 0.(x+1)` minor bump; see [ADR-0021](docs/architecture/decisions/0021-release-versioning-policy.md).

```sh
go get github.com/helmedeiros/bre-go@v0.14.0
```
