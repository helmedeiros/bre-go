...
- **`engine/indexed.Engine`.** Stable since v0.8.0; widened in v0.9.0. Sub-linear first-match adapter. Rules carry `Match parser.Condition`. Indexable shapes: `StringCondition{Op: OpEq}` (since v0.8.0) and `SetCondition{Op: OpIn}` (since v0.9.0, via bucket fan-out at AddRule). Fields omitted from `Match` are wildcarded via key-set walking. AddRule rejects non-indexable shapes with `ErrNonIndexableCondition`; fan-outs above `maxFanout` (1024) with `*FanoutTooLargeError`. Execute is O(K) hash lookups where K is the number of distinct key-sets registered. Live perf-bar tests gate any future change that would weaken the headline claim.
...
| [`engine/indexed`](engine/indexed) | Sub-linear first-match. Rules are bucketed by their key-set + value-tuple; `Execute` runs O(K) hash lookups where K is the number of distinct key-sets. As of v0.9.0, the indexable shape includes `parser.StringCondition{Op: OpEq}` and `parser.SetCondition{Op: OpIn}` (set-membership); a rule may also omit fields entirely (wildcard semantics). Input must be `map[string]string` or `map[string]interface{}`. | You have a large rule set (hundreds to tens of thousands of rules) shaped as routing-table / decision-table equality or set-membership matches, and per-call latency matters. See [`BENCHMARKS.md`](BENCHMARKS.md) for the cleared v0.8.0 and v0.9.0 success bars. |
