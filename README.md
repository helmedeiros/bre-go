...
`v0.10.0` -- tenth minor release. Widens `engine/indexed` further: admits `StringCondition{Op: OpNeq}` and `SetCondition{Op: OpNotIn}` as post-filter terms (paired with at least one indexable term), and ships a value-expression mini-grammar in `engine/parser` (`!value`, `value1|value2`, `*`) for CSV-shaped rule loaders. Two new v0.10.0 success-bar cells clear at 111× / 2 942× firstmatch; v0.8.0 + v0.9.0 bars continue to gate. Second of five Phase-4 follow-ups closing the gap toward production-grade matcher shapes (see [ADR-0035](docs/architecture/decisions/0035-indexed-negation-and-value-expression-syntax.md)). Four concrete adapters, rule loading from CSV and JSON, twelve public packages plus one test-only sibling, thirty-five ADRs on `main`. SemVer: pre-1.0 means breaking changes are still allowed but land as a `0.x → 0.(x+1)` minor bump; see [ADR-0021](docs/architecture/decisions/0021-release-versioning-policy.md).

```sh
go get github.com/helmedeiros/bre-go@v0.10.0
```
...
- **`engine/indexed.Engine`.** Stable since v0.8.0; widened in v0.9.0 and v0.10.0. Sub-linear first-match adapter. Rules carry `Match parser.Condition`. Indexable shapes (since v0.8.0 / v0.9.0): `StringCondition{OpEq}`, `SetCondition{OpIn}` via bucket fan-out. Post-filter shapes (since v0.10.0): `StringCondition{OpNeq}`, `SetCondition{OpNotIn}` -- admitted only when the rule also has at least one indexable term, return `ErrNoIndexableTerms` if the rule is pure-negation. Fields omitted from `Match` are wildcarded via key-set walking. AddRule rejects unknown shapes with `ErrNonIndexableCondition`; fan-outs above `maxFanout` (1024) with `*FanoutTooLargeError`. Execute is O(K) hash lookups + per-candidate post-filter evaluation. Live perf-bar tests gate any future change that would weaken the headline claim.
...
| [`engine/indexed`](engine/indexed) | Sub-linear first-match. Rules are bucketed by their key-set + value-tuple; `Execute` runs O(K) hash lookups where K is the number of distinct key-sets. The indexable shape is `StringCondition{OpEq}` (v0.8.0) and `SetCondition{OpIn}` (v0.9.0, via fan-out); `StringCondition{OpNeq}` and `SetCondition{OpNotIn}` (v0.10.0) admitted as post-filters on bucket hits. Wildcard semantics via field omission. Input must be `map[string]string` or `map[string]interface{}`. | You have a large rule set (hundreds to tens of thousands of rules) shaped as routing-table / decision-table equality / set / negation matches, and per-call latency matters. See [`BENCHMARKS.md`](BENCHMARKS.md) for the cleared v0.8.0 / v0.9.0 / v0.10.0 success bars. |
