...
`v0.18.0` -- eighteenth minor release. Adds the **metrics port + decorator** (hexagonal): `observability.ExecutionMetric` typed event + `observability.ExecutionMetricSink` interface (bre-go owns the contract) + `observability/metrics.Wrap` decorator. Backends (OTel, Prometheus, custom in-house) adapt to the port; bre-go core never imports any external metrics SDK. Cancellation semantics from v0.17 carry forward in the data model: `Canceled` + `CancelReason` are separate from `Err`. The OTel metric adapter ships in v0.19.0 when upstream OTel's metric SDK stabilizes. ADR-0043's hexagonal claim was tested with a pre-tag scientific review at [`scientific/v0.18.0/REPORT.md`](scientific/v0.18.0/REPORT.md): three independent sinks (channel fanout, atomic counters, sliding-window percentiles) written from scratch at ~50 LOC each. Forty-three ADRs on `main`. SemVer: pre-1.0 means breaking changes are still allowed but land as a `0.x → 0.(x+1)` minor bump; see [ADR-0021](docs/architecture/decisions/0021-release-versioning-policy.md).

```sh
go get github.com/helmedeiros/bre-go@v0.18.0
```
