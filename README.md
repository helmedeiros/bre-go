...
`v0.15.0` -- fifteenth minor release. Adds `engine/indexed.Engine.ExportSnapshot()` and `LoadSnapshot()` -- a JSON-serializable snapshot of an indexed engine's rule set, plus a matching loader that produces an already-Built engine ready to Execute. The artifact is content-addressable (byte-identical across independent builds from the same source), portable across CPU architectures (verified arm64 ↔ amd64), and tolerant of adversarial round-trip (deep AND nesting, unicode, IEEE-754 infinity bounds, large OpIn fan-out). Action callbacks attach out-of-band by rule name at Load time. **The format is not a load-time optimization** vs. CSV + the bre-go parser: at 10k rules the snapshot path is roughly 2× *slower* than the source-build baseline because verbose tagged-union JSON costs more to decode than the parser costs to run; see [`scientific/v0.15.0/REPORT.md`](scientific/v0.15.0/REPORT.md) for the measurements. Four concrete adapters, rule loading from CSV and JSON, twelve public packages plus one test-only sibling, forty ADRs on `main`. SemVer: pre-1.0 means breaking changes are still allowed but land as a `0.x → 0.(x+1)` minor bump; see [ADR-0021](docs/architecture/decisions/0021-release-versioning-policy.md).

```sh
go get github.com/helmedeiros/bre-go@v0.15.0
```
